...
export const makeBadge = (hits: number) => {
  ...
  const width = `Profile Views: ${formatted}`.length * 6.5;
  const svg = `
  <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${
    width + 10
  }" height="20" role="img" aria-label="Profile Views: ${formatted}}">
    <title>Profile Views: ${formatted}}</title>
    <g shape-rendering="crispEdges">
      <rect x="0" width="${width + 50}" height="20" fill="black"/>
    </g>
